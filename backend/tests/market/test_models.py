...
class TestPriceUpdate:
    ...
    def test_price_update_creation(self):
        ...
        update = PriceUpdate(
            ...
            session_start_price=185.00, timestamp=1234567890.0,
        )
        assert update.ticker == "AAPL"
        assert update.price == 190.50
        assert update.previous_price == 190.00
        assert update.session_start_price == 185.00
    ...
    def test_change_calculation(self):
        ...
        update = PriceUpdate(
            ticker="AAPL", price=190.50, previous_price=190.00,
            session_start_price=190.00, timestamp=1234567890.0,
        )
    ...
    def test_change_percent_up(self):
        ...
        update = PriceUpdate(
            ...
            session_start_price=100.00, timestamp=1234567890.0,
        )
    ...
    def test_change_percent_down(self):
        ...
        update = PriceUpdate(
            ...
            session_start_price=200.00, timestamp=1234567890.0,
        )
    ...
    def test_change_percent_zero_previous(self):
        ...
        update = PriceUpdate(
            ...
            session_start_price=0.00, timestamp=1234567890.0,
        )
        assert update.change_percent == 0.0

    def test_session_change_percent_up(self):
        """Test session change percent when price has risen from session start."""
        update = PriceUpdate(
            ticker="AAPL", price=200.00, previous_price=199.00,
            session_start_price=190.00, timestamp=1234567890.0,
        )
        assert update.session_change_percent == round((200.00 - 190.00) / 190.00 * 100, 4)

    def test_session_change_percent_down(self):
        """Test session change percent when price is below session start."""
        update = PriceUpdate(
            ticker="AAPL", price=180.00, previous_price=181.00,
            session_start_price=190.00, timestamp=1234567890.0,
        )
        assert update.session_change_percent == round((180.00 - 190.00) / 190.00 * 100, 4)

    def test_session_change_percent_zero_start(self):
        """Test session change percent with zero session start price."""
        update = PriceUpdate(
    ...
    def test_to_dict(self):
        ...
        assert result["session_start_price"] == 185.00
        assert result["timestamp"] == 1234567890.0
        assert result["change"] == 0.50
        assert result["change_percent"] == 0.2632  # (0.50 / 190.00) * 100
        assert result["session_change_percent"] == round((190.50 - 185.00) / 185.00 * 100, 4)
